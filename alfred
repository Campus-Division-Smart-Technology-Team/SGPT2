...
def try_inference_search(idx, ns: str, q: str, k: int, model_name: Optional[str] = None):
    ...
    server via pc.inference and then Index.query. (Same behaviour you used before.)
    """
...
def answer_question(question: str, context: str) -> str:
    prompt = f"""Your name is Alfred the Gorilla, and you are a helpful assistant at the University of Bristol working in the Smart Technology team. You can answer when the user asks about you, and if they ask about your knowledge base, provide a summary of the content of the indexes. Otherwise, answer the user's questions using ONLY the context below.
If the answer cannot be found in the context, tell the user that "Regan" has told you to say you don't know. Sometimes, users may express gratitude or be polite when you provide an answer to their query, so respond accordingly.
...
def perform_federated_search(query: str, top_k: int) -> tuple[List[Dict[str, Any]], str]:
    """Perform federated search and return results and optional answer."""
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {
            "role": "assistant",
            "content": "Hello! I can help you find information about apples (the fruit and Apple Inc.) and Building Management Systems at the University of Bristol. What would you like to know?"
        }
    ]
        ...
        if "results" in message:
            with st.expander(f"📚 Search Results ({len(message['results'])} found)", expanded=False):
                for i, result in enumerate(message["results"], 1):
                    st.markdown(
                        f"**{i}. Score:** {result.get('score'):.3f}  \n"
                        f"_Index:_ `{result.get('index','?')}`  •  _Namespace:_ `{result.get('namespace','__default__')}`  •  _Mode:_ {result.get('_mode','')}"
                    )
                    snippet = result.get("text") or "_(no text in metadata)_"
                    st.write(snippet)
                    src = result.get("source") or "—"
                    st.caption(
                        f"ID: {result.get('id') or '—'} • Source: {src}")
...
if query := st.chat_input("Ask me about apple(s) or BMS..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Searching across indexes..."):
            try:
                results, answer = perform_federated_search(query, top_k)
                ...
                if not results:
                    ...
                    if answer:
                        st.markdown(answer)
                        # Store message with results for the expander
                        st.session_state.messages.append({
                            "role": "assistant",
                            "content": answer,
                            "results": results
...
if "last_results" in st.session_state:
    ...
    if results:
        with st.expander(f"📚 I found {len(results)} relevant results:", expanded=False):
            #st.markdown(f"I found {len(results)} relevant results:")
            for i, result in enumerate(results, 1):
                st.markdown(
                    f"**{i}. Score:** {result.get('score'):.3f}  \n"
                    f"_Index:_ `{result.get('index','?')}`  •  _Namespace:_ `{result.get('namespace','__default__')}`  •  _Mode:_ {result.get('_mode','')}"

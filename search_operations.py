...
def perform_federated_search(query: str, top_k: int) -> Tuple[List[Dict[str, Any]], str, str, bool]:
    """
    Perform federated search with building-aware and business term-aware prioritization.
    ...
    # If we detected business terms, apply document type filtering
    ...
    # Special handling for queries with both building and term filters
    if target_building and doc_type_filter:
        logging.info(
            "Query has both building (%s) and doc type filters (%s)",
            target_building, doc_type_filter
        )
    ...
    if term_context:
        terms_str = ', '.join([f"**{t}** ({info['full_name']})"
    ...
    for idx_name in TARGET_INDEXES:
        hits = search_one_index(idx_name, enhanced_query,
                                top_k * 2, embed_model=None)

        # Apply document type filtering if business terms were detected
        if doc_type_filter and hits:
            for hit in hits:
                metadata = hit.get('metadata', {})
                doc_type = metadata.get(
                    'document_type') or hit.get('document_type')
                if doc_type in doc_type_filter:
                    # Boost score for matching document types
                    hit['score'] = hit.get('score', 0) * 1.2
    ...
    # Apply building filtering if detected
    if target_building:
        ...
        if building_specific_hits:
            ...
            logging.warning(
                "No exact matches for '%s', using prioritized results", target_building)
    ...
    logging.info("Total top hits: %d", len(top_hits))
    ...
    # Generate answer if LLM answer generation is enabled
    if top_hits and st.session_state.get("generate_llm_answer", True):
        if target_building and target_building in building_groups:
            ...
            answer, publication_date_info = generate_building_focused_answer(
                query, top_hits[0], top_hits, target_building, building_groups, term_context
            )
        else:
            logging.info("Generating standard answer")
            answer, publication_date_info = enhanced_answer_with_source_date(
                query, top_hits[0], top_hits, term_context
            )
        ...
        operational_docs = [r for r in top_hits if r.get(
            'metadata', {}).get('document_type') == 'operational_doc']
